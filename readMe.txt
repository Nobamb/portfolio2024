...
배포 링크: https://mugunghwaedu.modoo.at/
...
유효성 검증(Validation) 로직 구현: 예약 시스템에서 Date 객체와 hasClass를 활용해 과거 날짜 선택 제한 및 알림창 예외 처리 구현. 예약 인원수 입력 폼에 정규식/유효성 검사 매커니즘을 적용하여 범위 초과(0 이하, 11 이상) 및 소수점, 문자 입력 원천 차단.
...
배포 링크: https://pokemonkorea.co.kr/
...
🌟 핵심 구현 및 기술적 접근
전역 상태 동기화 (오디오 스트리밍 아키텍처):

Intro ➡️ 메인 ➡️ 앨범 등 서로 다른 섹션(Page 1, Page 4)에 존재하는 오디오 플레이어 간의 싱크를 맞추어, 한쪽에서 재생 시 반대편 소스가 정지하고 상태 바(Progress Bar)가 실시간 업데이트되는 오디오 스크립트 설계.
...
Design & Prototyping: Figma (85%), Photoshop (80%) 기반의 와이어프레임 설계 및 컴포넌트 시스템 구축 가능.

Frontend Foundations: HTML5/CSS3 (90%) 지식을 바탕으로 시맨틱 마크업 및 웹 표준, 미디어 쿼리를 활용한 완벽한 반응형 레이아웃 구현.

Scripting & Logic: JavaScript/jQuery (80%)를 활용한 풍부한 웹 인터랙션 연출 및 가용한 DOM 데이터 조작, 유효성 검사 로직 설계 가능.
